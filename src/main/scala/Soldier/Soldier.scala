...
abstract class Soldier(canHitBack: Boolean = false,
                       hitValue: Int,
...
case class Footer(hitValue: Int = 3, myLife: Int = 7, myCommand: Command, side: Side.Value)
  extends Soldier(hitValue = hitValue, myLife = myLife, command = myCommand, side = side) with CanHit {
...
case class Knight(hitValue: Int = 5, myLife: Int = 10, myCommand: Command, side: Side.Value)
  extends Soldier(hitValue = hitValue, myLife = myLife, canHitBack = true, command = myCommand, side = side)
    with CanHit {
...
case class Archer(hitValue: Int = 2, myLife: Int = 5, myCommand: Command, side: Side.Value)
  extends Soldier(hitValue = hitValue, myLife = myLife, command = myCommand, side = side)
